...
public class SortTest<> {
    ...
    private SortingStudentByGPA[] GPASort;

    @Before
    public void setUp() {
        iDNumber = new Student[5];
        GPASort = new SortingStudentByGPA[5];
        for (int i = 0; i < iDNumber.length; i++) {
            iDNumber[i] = new Student("Student" + i, (int) (Math.random() * 15 + 1), (int) (Math.random() * 200 + 1));
            GPASort[i] = new SortingStudentByGPA("Student" + i, (int) (Math.random() * 15 + 1), (int) (Math.random() * 200 + 1));
        }
    }
    ...
    public void quickSortByGPATest() {
        SortingStudentByGPA[] GPASortTest;
        GPASortTest = GPASort;
    ...
    public static void quickSort(SortingStudentByGPA[] array, int low, int high) {
        ...
        SortingStudentByGPA support = array[middle];
        ...
        while (i <= j) {
            ...
            if (i <= j) {
                SortingStudentByGPA temp = array[i];
                array[i] = array[j];
                array[j] = temp;
                i++;
                j--;
